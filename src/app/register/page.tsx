...
export default function RegisterPage() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!tenantName.trim()) {
      errors.tenantName = t.requiredField;
    }
              ...
              <motion.div {...fieldAnimation(0.55)} className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className={`ps-10 h-11 ${localErrors.tenantName ? 'border-destructive focus-visible:ring-destructive/30' : ''}`}
                    disabled={isLoading}
                  />
                </div>
                {localErrors.tenantName && (
                  <p className="text-xs text-destructive mt-1">{localErrors.tenantName}</p>
                )}
              </motion.div>
